...
export default function Leaderboard() {
  ...
  useEffect(() => { const controller = new AbortController(); fetchResource(leaderboardEndpoint, controller.signal).then((payload) => setEntries(getResourceItems(payload))).catch((reason) => { if (reason.name !== 'AbortError') setError(reason.message) }); return () => controller.abort() }, [])
