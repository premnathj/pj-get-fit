...
export default function Teams() {
  ...
  useEffect(() => { const controller = new AbortController(); fetchResource('teams', controller.signal).then((payload) => setTeams(getResourceItems(payload))).catch((reason) => { if (reason.name !== 'AbortError') setError(reason.message) }); return () => controller.abort() }, [])
