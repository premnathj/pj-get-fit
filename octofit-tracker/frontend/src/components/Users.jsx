...
export default function Users() {
  ...
  useEffect(() => { const controller = new AbortController(); fetchResource('users', controller.signal).then((payload) => setUsers(getResourceItems(payload))).catch((reason) => { if (reason.name !== 'AbortError') setError(reason.message) }); return () => controller.abort() }, [])
