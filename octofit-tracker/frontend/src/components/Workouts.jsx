...
export default function Workouts() {
  ...
  useEffect(() => { const controller = new AbortController(); fetchResource(workoutsEndpoint, controller.signal).then((payload) => setWorkouts(getResourceItems(payload))).catch((reason) => { if (reason.name !== 'AbortError') setError(reason.message) }); return () => controller.abort() }, [])
